...
def save_images(webpage, images, names, image_path, aspect_ratio=1.0, width=256):
    ...
    # 提取 image_path 的相对路径，并去除文件名
    subfolder_path = os.path.dirname(image_path)
    full_image_dir = os.path.join(image_dir, subfolder_path)
    ...
    for label, im_data in zip(names, images):
        ...
        # 将图片保存到子文件夹中
        save_path = os.path.join(full_image_dir, image_name)
        util.save_image(im, save_path, aspect_ratio=aspect_ratio)
        
        # 添加到网页显示
        ims.append(os.path.join(subfolder_path, image_name))  # 使用相对路径
        txts.append(label)
        links.append(os.path.join(subfolder_path, image_name))

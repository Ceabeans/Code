...
def parse_args():
    ...
    parser.add_argument('--num_ids', type=int, default=3739)             
    parser.add_argument('--samples', type=int, default=10)              
    parser.add_argument('--nproc', type=int, default=1)                 
    parser.add_argument('--imsize', type=int, default=256)              
    parser.add_argument('--output', type=str, default='./BEZIER')    # Output file path
...
def generate_parameters_1():                          
    ...
    head1, thead1 = sample_edge(0, 0.3)             
    ...
    if np.random.uniform() <= 0.5:
        ...
        head2, thead2 = sample_edge(-0.23, -0.08)   
        head3, thead3 = sample_edge(-0.5, -0.25)    
    tail2, ttail2 = sample_edge(1.8, 2.15)          #(0.85,1)-（1,0.8）
    tail3, ttail3 = sample_edge(2.3, 2.7)           #(0.3,1)-（0.7,1）

    if np.random.uniform() <= 0.7:
        head4, thead4 = sample_edge(0, 0.2)         
        if ttail3 <= 2.5:                           
...
def generate_parameters_2():                        
    ...
    if np.random.uniform() <= 0.7:
        head4, thead4 = sample_edge(0, 0.2)         
        if ttail3 <= 2.5:                           
            tail4, ttail4 = sample_edge(2.1, 2.5)   
        else:
            tail4, ttail4 = sample_edge(2.5, 2.7)   
    else:
        head4, thead4 = sample_edge(2.4, 3)         
        tail4, ttail4 = sample_edge(1.2, 1.8)       
...
def batch_process(proc_index, ranges, args):
    ...
    for id_idx, i in enumerate(range(*ranges[proc_index])):
        ...
        for s in range(args.samples):
            ...
            coord2_ = coord2 + np.random.uniform(-15, 15, coord2.shape)
            s2_ = s2 + np.random.uniform(-0.1, 0.1, s2.shape)
            t2_ = t2 + np.random.uniform(-0.2, 0.2, s2.shape)
        ...
        print("proc[%.2d/%.2d] id=%.4d [%.4d/%.4d]  (%.3f sec per id)" % (proc_index, args.nproc, i, id_idx, ids_per_proc, average_meter.avg))  

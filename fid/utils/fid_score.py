...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=8):
    if os.path.exists(path + '/fid_stat.npz'):
        with np.load(path + '/fid_stat.npz') as f:
            m, s = f['mu'][:], f['sigma'][:]
    else:
        path = pathlib.Path(path)
        files = sorted([file for ext in IMAGE_EXTENSIONS
                        #    for file in path.glob('*.{}'.format(ext))])    # glob方法无法获取子文件夹下文件
                       for file in path.rglob('*.{}'.format(ext))])
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device, num_workers)
        np.savez(str(path) + '/fid_stat.npz', mu=m, sigma=s)
...
def save_statistics_of_path(path, out_path, device=None, batch_size=50, dims=2048, num_workers=8):
    ...
    np.savez(out_path + '/fid_stat.npz', mu=m1, sigma=s1)


def calculate_fid_given_paths(paths, device=None, batch_size=64, dims=2048, num_workers=0):
